...
/* Loading an images can fail for several reasons. Try to log an
...
function error(url, exception) {
    console.log("Unable to load images ", url, ". It's probably missing or you've run into a CORS issue.");
    if (exception)
...
Raster.prototype.init = function(callback) {
     ...
     * don't bother with the potential images / XHR request */
    var canvas = document.createElement("canvas");
...
function ShapeInfo(element) {
    ...
    var parserSettings = {
        ...
        shapeImageThreshold: element.getAttribute('data-shape-images-threshold')
    };
...
// See http://dev.w3.org/csswg/scss-shapes/#margin-box
...
StylePolyfill.prototype.onStylesLoaded = function(stylesheets) {
    ...
    var rules = [], properties = ["shape-outside", "shape-margin", "shape-images-threshold"];
    properties.forEach(function(property) {
